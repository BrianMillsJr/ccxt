...
class cryptocom extends Exchange {
    ...
    public function describe() {
        return $this->deep_extend(parent::describe (), array(
            ...
            'name' => 'Cryptocom',
            'countries' => array( 'UK' ),
            'rateLimit' => 3600, // 10k calls per hour
            'version' => 'v2',
            ...
            'urls' => array(
                'logo' => 'https://crypto.com/exchange/img/logo.76653258.svg',
                #'api' => 'https://uat-api.3ona.co', # dev
                'api' => 'https://api.crypto.com', # prod
                'www' => 'https://crypto.com',
                'doc' => 'https://exchange-docs.crypto.com',
                'fees' => 'https://crypto.com/exchange/document/fees-limits',
                'referral' => 'https://crypto.com/exch/msnjc47tqf',
            ),
...
#print_r(PHP_EOL.'- '.__METHOD__);
        ...
        $data = $this->safe_value($currencies, 'instruments', array());
        //Array
        //(
        //    [0] => Array
        //        (
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $margin = $this->safe_value($market, 'margin_trading_enabled', 1) ? true : false;
            $result[] = array(
                'id' => $id,
                'symbol' => $symbol,
                'base' => $base,
        ...
        return $result;
        #return $this->set_markets($result);
    }
...
#print_r(PHP_EOL.'- '.__METHOD__);
    ...
    public function parse_ticker($data) {
        if (!isset($this->marketsById[$data['i']])) {
            return null;
        }
        $market = $this->marketsById[$data['i']];
        $symbol = $market['symbol'];
        $timestamp = $this->safe_float($data, 't');
        $bid = $this->safe_float($data, 'b');
        $ask = $this->safe_float($data, 'k');
        $last = $this->safe_float($data, 'a');
...
#print_r(PHP_EOL.'- '.__METHOD__);
        ...
        $tickers = $this->publicGetPublicGetTicker();
        $result = $this->safe_value($tickers, 'result');
...
#        print_r($results);
        ...
        #$return = ['BTC/USDT' => $this->fetch_ticker('BTC/USDT')];
        $return = ['CRO/BTC' => $this->fetch_ticker('CRO/BTC')];
        print_r ($return);
        return $return;
    }
...
*/
        $ticker = $this->publicGetPublicGetTicker(['instrument_name' => $market['id']]);

	        $result = $this->safe_value($ticker, 'result');
	        $data = $this->safe_value($result, 'data');
